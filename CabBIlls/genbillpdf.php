...
//Connecting to the database
 $dbhandle = mysqli_connect("localhost", "root", "mysql","cabBills")
  ...
  $exp = $_POST['exp'];
                    $brdtm1 = $_POST['brdtm1'];
                     $brdtm2 = $_POST['brdtm2'];
                    $amt1 = (double)$_POST['amt1'];
                     $amt2 = (double)$_POST['amt2'];
                    
                      $rate1 = (double)$_POST['rate1'];
                     $rate2 = (double)$_POST['rate2'];
                     
                     $dateval = $_POST['dateval'];
                     
                      $sql1="insert into employee values('".$dateval."','".$brdtm1."',".$amt1.",".$rate1.")";
                    // echo $sql; 
                                                   mysqli_query($dbhandle,$sql1);
                     $sql2="insert into employee values('".$dateval."','".$brdtm2."',".$amt2.",".$rate2.")";
...
while($row = mysqli_fetch_array($result,MYSQLI_NUM)) 
{
    ...
    $dbrate=$row{3};
	   ...
	   if(strcmp($brdtm,"Morning")==0)
		   $cabstr="Cab Pickup Charges";
	    else
			$cabstr="Cab Drop Charges";
	$column_itemno = $column_sno.$item."\n";
...
$pdf->SetTextColor(0,15,255);
$pdf->Cell(0,6,'Business Expense Report',0,0,'C');

$pdf->SetY(42);
$pdf->SetX(8);

$pdf->SetFont('Times','B',12);
$pdf->Cell(0, 4, "Employee Name",0,0, 'C');
$pdf->SetFont('Times','B',12);
$pdf->SetX(43);
$pdf->Cell(0,4, $name, 0,0,'C');
$pdf->SetX(109);
$pdf->Cell(20,4,'Payment Date',0,0,'C');
$pdf->SetX(144);
$pdf->Cell(0,4, $pmtdt, 0,0,'C');

$pdf->SetY(48);
$pdf->SetX(8);
$pdf->Cell(0,4,"Payment", 0,0,'C');
$pdf->SetX(43);
$pdf->Cell(0,0.25, "Transportation", 0,0,'C');
$pdf->SetX(109);
$pdf->Cell(20,4,'Manager',0,0,'C');
$pdf->SetX(144);
$pdf->Cell(0,4, $manager, 0,0,'C');

$pdf->SetY(54);
$pdf->SetX(8);
$pdf->Cell(0,4,"Expenses Nature", 0,0,'C');
$pdf->SetX(43);
$pdf->Cell(0,0.25, "Cab Charges - ".$exp, 0,0,'C');
$pdf->SetX(109);
$pdf->Cell(20,4,'Bank Account Number',0,0,'C');
$pdf->SetX(144);
$pdf->Cell(0,4, $acc, 0,0,'C');

$pdf->SetY(60);
$pdf->SetX(8);
$pdf->Cell(0,4,"Project name/Expenses Total", 0,0,'C');
$pdf->SetX(43);
$pdf->Cell(0,0.25, $aed, 0,0,'C');
$pdf->SetX(109);
$pdf->Cell(20,4,'Finance',0,0,'C');
$pdf->SetX(144);
$pdf->Cell(0,4, " ", 0,0,'C');
